"""
SAM.gov CRADA Opportunity Monitor
Polls SAM.gov for new/updated Cooperative Research and Development Agreement notices and posts to Teams.
"""
...
def collect_all_opportunities(posted_from: str, posted_to: str) -> list[dict]:
    """Fetch all records in window, keep only CRADA-titled ones, classify each."""
    raw = fetch_all_in_window(posted_from, posted_to)
    matches = []
    for opp in raw:
        if not is_crada_relevant(opp):
            continue
        opp["_matched_profile"] = classify_opportunity(opp)
        opp["_matched_emoji"]   = emoji_for_profile(opp["_matched_profile"])
        matches.append(opp)
    log.info("Relevance filter: %d CRADA matches out of %d records", len(matches), len(raw))
    return matches


def is_crada_relevant(opp: dict) -> bool:
    title    = (opp.get("title") or "").upper()
    combined = f" {title} "
    return any(term in combined for term in CRADA_TITLE_TERMS)
...
def classify_opportunity(opp: dict) -> str:
    ...
    if any(x in dept for x in ["NATIONAL INSTITUTES", "NIH", "NATIONAL INSTITUTE"]):
        return "NIH CRADA"
    if any(x in dept for x in ["NATIONAL LAB", "NATIONAL LABORATORY", "ARGONNE", "OAK RIDGE", "BROOKHAVEN",
                                "PACIFIC NORTHWEST", "NATIONAL RENEWABLE"]):
        return "National Lab CRADA"
...
def emoji_for_profile(label: str) -> str:
    mapping = {"DoD CRADA": "🔵", "NIH CRADA": "🟣", "National Lab CRADA": "🟠", "CRADA": "🟢"}
...
def ai_summarize(opp: dict) -> Optional[str]:
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "You summarize federal partnership opportunities in 2 sentences for a BD team. "
                    "Focus on: collaborative scope, partner eligibility, and timeline. Be concise."
                )},
                {"role": "user", "content": (
...
def format_teams_message(opp: dict, is_update: bool) -> dict:
    ...
    label      = opp.get("_matched_profile", "CRADA")
...
def post_run_summary(
    ...
    body = [
        _text("📊 SAM.gov CRADA Monitor — Run Complete", weight="Bolder", size="Medium"),
    ...
    if opportunities:
        branch_items = [_text("**By Agency**", weight="Bolder")]
